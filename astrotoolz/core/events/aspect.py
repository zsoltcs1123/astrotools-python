...
class Aspect(AstroEvent):
    ...
    def __repr__(self):
        ...
        return f"{self.dt}\t{self.angle.source.point}\t{self.coord_system}\t{self.asp_text.lower()} [{self.angle.abs_diff:.3f}] vs {self.angle.target.point} at [{self.angle.source.lon.decimal:.3f}]"
